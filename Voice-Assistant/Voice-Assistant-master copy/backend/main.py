...
def voice_chat():
    ...
    try:
        ...
        llm_text = None
        t0 = log_time("Start orchestrator call")
        try:
            llm_text = orchestrator_obj.handle_query(user_text, user_language=user_language_code)
        except Exception as e:
            logger.warning("Orchestrator failed, will fallback to Gemini: %s", e)
        t1 = log_time("End orchestrator call")
        print(f"Orchestrator duration: {t1-t0:.2f} seconds")
        if not llm_text or len(llm_text.strip()) < 2:
            t2 = log_time("Start Gemini fallback")
            ...
            t3 = log_time("End Gemini fallback")
            print(f"Gemini fallback duration: {t3-t2:.2f} seconds")
        # --- Language enforcement for TTS ---
        def enforce_language(text: str, target_lang: str) -> str:
            # If SarvamAI supports translation, use it here
            try:
                translated = sarvam_client.text_translate.translate(
                    text=text,
        ...
        tts_input = enforce_language(tts_input, user_language_code)
        if not tts_input:
